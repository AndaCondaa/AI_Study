...
- #### <b>Transfer Learning (전이 학습)</b>
  ...
  - 레이블링 여부 / 태스크 일치 여부 / 도메인 일치 여부가 중요한 기준

| 유형                              | 소스 도메인 (레이블링) | 타깃 도메인 (레이블링) |
| --------------------------------- | ---------------------- | ---------------------- |
| 귀납적 전이학습 (자기주도적 학습) | X                      | O                      |
| 귀납적 전이학습 (다중 작업 학습)  | O                      | O                      |
| 변환적 전이학습                   | O                      | X                      |
| 비지도 전이학습                   | X                      | X                      |
| 제로-샷 전이학습                  | O                      | X                      |
| 원-샷 전이학습                    | O                      | X                      |
...
- #### <b>Feature Extraction / Fine-tuning</b>

| 유형               | 데이터 | 유사도 | 백본 가중치 학습 여부                                        |
| ------------------ | ------ | ------ | ------------------------------------------------------------ |
| Feature Extraction | 적음   | 높음   | 동결하고 분류기만 학습                                       |
| Fine-tuning        | 많음   | 낮음   | 동결 X, 전체 재적응<br>(구조만 사용한다는 것은 아님)         |
| Fine-tuning        | 적음   | 낮음   | 하위 계층 일부만 동결 <br>(저수준 영역의 가중치만 그대로 사용) |
| Fine-tuning        | 많음   | 높음   | 상위 계층빼고 모두 동결<br>(데이터 많지만, 유사도가 높으므로, 유사도가 높아도 달라질 수 있는<br> 상위계층만 추가 학습, 분류기도 추가학습) |
...
- 기본 개념
  ...
  - 패딩 : 합성곱 신경망에서 출력 크기를 조정할 목적
    - 패딩이 없으면 가장자리는 연산에 관여하는 횟수가 상대적으로 적음
  - 스트라이드(보폭) : 필터를 적용하는 위치의 간격
    - 간격을 조절해서 모델 매개변수의 수를 감소시킬 수 있으며, 모델의 복잡도를 낮추고 과대적합을 방지할 수 있음
  - 입출력 크기
  ...
  - 팽창 (Dilation)
    ...
    - 팽창이 2인 경우 입력 데이터를 한칸 씩 건너뛰면서 합성곱 수행
    - 필터의 크기를 키우지 않고 입력 데이터에 더 넓은 영역을 고려할 수 있게 함
    - 모델의 매개변수 수를 줄일 수 있음
    - 팽창값이 너무 크면 인접한 픽셀값을 고려하지 않게 되어 공간적인 정보가 보존되지 않아서 특징 추출 효과가 떨어짐
- 3차원 데이터 처리
...
- 풀링 계층
  ...
  - 최대 풀링 / 평균 풀링
    - 주로 최대 풀링을 사용
  - <b>어찌됐던 공간적 정보가 소실되기 때문에 최근에는 연산량을 늘리더라도 풀링을 하지 않고 학습함</b>
